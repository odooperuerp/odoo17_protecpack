...
class AccountInvoiceRetentionLines(models.Model):
	...
	amount_paid_retention = fields.Monetary(string="Monto Retenido",
		currency_field="retention_company_currency_id",readonly=True)
	...
	nro_receipt_retention=fields.Char(string="Nro Comprobante Retención",readonly=True)

	paid_journal_id = fields.Many2one('account.journal',string="Diario de Cobro",readonly=True)
	retention_ref = fields.Char(string="Referencia",readonly=True)
	operation_number = fields.Char(string="Número de Operación",readonly=True)
