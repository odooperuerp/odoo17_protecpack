...
class AccountMoveLine(models.Model):
    ...
    def get_account_me_id(self,move_line_id,account_type):

        new_term_account = False

        move = move_line_id.move_id
        ...
        if move.partner_id:
            ...
            if account_type=='asset_receivable':
                ...
                if move.currency_id and move.currency_id != move.company_id.currency_id:
                    new_term_account = move.commercial_partner_id.property_account_receivable_me_id

            elif account_type=='liability_payable':

                if move.currency_id and move.currency_id != move.company_id.currency_id:
                    if move.l10n_latam_document_type_id.code=='02':
                        new_term_account = move.commercial_partner_id.property_account_payable_me_fees_id
                    else:
                        new_term_account = move.commercial_partner_id.property_account_payable_me_id
                else:
                    if move.l10n_latam_document_type_id.code=='02':
                        new_term_account = move.commercial_partner_id.property_account_payable_fees_id

        return new_term_account and new_term_account.id or False
    ...
    def _compute_account_id(self):
        ...
        if term_lines:
            ...
            for line in term_lines:
                ...
                _logger.info('\n\nEL HONGOSH EL HONGOSH 3333 !!!\n\n')

                account_type = 'asset_receivable' if line.move_id.is_sale_document(include_receipts=True) else 'liability_payable'
                move = line.move_id

                new_term_account = self.get_account_me_id(line,account_type)
                _logger.info('\n\nCUENTA EN ME\n\n')
                _logger.info(new_term_account)

                account_id = new_term_account or (
