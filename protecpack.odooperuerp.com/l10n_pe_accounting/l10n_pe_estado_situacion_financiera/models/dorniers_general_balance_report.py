...
class DorniersGeneralBalanceReport(models.Model):
	...
	dorniers_general_balance_report_line_ids = fields.One2many('dorniers.general.balance.report.line',
		'dorniers_general_balance_report_id', string="Rubros del Estado de Situación Financiera", readonly=True)
	...
	template_general_balance_report_id = fields.Many2one('template.general.balance.report', 
		string="Plantilla Estado de Situación Financiera",
		...
		for rubro in rubros_activos_corrientes:
			...
			if rubro.calculation_type=='accounts':
				...
				if rubro.account_ids:
					suma= self.query_balance_of_sums_and_balances(
						rubro.account_ids.mapped('id'),self.fecha_inicio.strftime('%Y-%m-%d'),self.fecha_final.strftime('%Y-%m-%d'))
	...
	def _generate_xlsx(self, output):
		...
		ws.merge_range('C1:I2','ESTADO DE SITUACIÓN FINANCIERA-BALANCE GENERAL ' \
			+ 'DEL ' + self.fecha_inicio.strftime('%d-%m-%Y') + '  AL ' + self.fecha_final.strftime('%d-%m-%Y'),titulo1)
	
		ws.merge_range('A6:D6','EJERCICIO:', titulo_1)
		ws.merge_range('E6:F6',self.fecha_final.strftime('%Y') or '', titulo_1)
		
		ws.merge_range('A7:D7','RUC:', titulo_1)
		ws.write('E7:F7',self.company_id.vat or '', titulo_1)

		ws.merge_range('A8:D8','APELLIDOS Y NOMBRES, DENOMINACIÓN O RAZÓN SOCIAL:', titulo_1)
		ws.merge_range('E8:F8',self.company_id.name or '', titulo_1)
		...
		ws.merge_range("A10:E10","ACTIVO",titulo2_center)

		ws.merge_range("A11:C11",'ACTIVO CORRIENTE',titulo2_left)
		ws.merge_range("D11:E11",'',titulo2_left)
		...
		ws.merge_range("F10:J10","PASIVO",titulo2_center)

		ws.merge_range("F11:H11",'PASIVO CORRIENTE',titulo2_left)
		ws.merge_range("I11:J11",'',titulo2_left)
