...
class  AccountRendicionGastosLine(models.Model):
	_name = 'account.rendicion.gastos.line'
	...
	rendicion_gastos_id = fields.Many2one('account.rendicion.gastos',string="Documento a Rendir",
	...
	@api.depends(
		...
		'invoice_currency_id',
		'rendicion_gastos_id',
		'rendicion_gastos_id.company_id',
		'rendicion_gastos_id.company_id.currency_id',
	...
	def compute_campo_amounts(self):
		for rec in self:
			if rec.invoice_id:
				...
				residual_signed = rec.invoice_id.amount_residual
				residual_company_signed = rec.invoice_id.amount_residual_signed
				amount_currency = rec.invoice_id.amount_total or 0.00
				rec.balance_sign = +1 if rec.invoice_id.amount_total_signed >=0.00 else -1
				...
				if rec.invoice_currency_id and (rec.invoice_currency_id != rec.company_currency_id):
					...
					rec.amount_total = abs(rec.invoice_id.amount_total_signed)
	...
	def compute_campo_amount_total_rendir_company_currency(self):
		for rec in self:
			if rec.rendicion_gastos_id.rendicion_date and rec.invoice_id:
				if rec.invoice_currency_id and rec.invoice_currency_id != rec.company_currency_id:
