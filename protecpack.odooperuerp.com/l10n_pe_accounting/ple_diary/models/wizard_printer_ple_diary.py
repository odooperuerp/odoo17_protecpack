...
class WizardPrinterPleDiary(models.TransientModel):
	...
	def file_name(self, file_format):
		...
		if self.ple_diary_id.periodo:
			file_name = "LE%s%s%s00%s%s%s1.%s" % (self.ple_diary_id.company_id.vat, self.ple_diary_id._periodo_fiscal(),
									...
									self.ple_diary_id.currency_id.code_sunat or '1', file_format)
		elif self.fecha:
	...
	def get_initial_values(self):
		...
		for line in records:
			...
			account_dic[line['account_id']][0] += line['debit_saldo_inicial']
			account_dic[line['account_id']][1] += line['credit_saldo_inicial']
		return account_dic
	...
	def _generate_ledger_xlsx(self, output):
		...
		ws.merge_range('A1:C1','FORMATO 6.1: LIBRO MAYOR',titulo1)
		...
		def total_balances(row):
			...
			if current_balance >= 0:
				ws.write(row,7,abs(round(current_balance,2)),titulo_1)
			else:
				ws.write(row,8,abs(round(current_balance,2)),titulo_1)
			return row
		...
		for line in lines:
			if line.codigo_cuenta_desagregado_id.id != initial_accounting_entry:
				...
				previous_balance = initial_values.get(line.codigo_cuenta_desagregado_id.id, False)

				if previous_balance:
					previous_balance = previous_balance[0] - previous_balance[1]
					if previous_balance >= 0:
						ws.write(row,7,abs(round(previous_balance,2)) or '', titulo_1)
					else:
						ws.write(row,8,abs(round(previous_balance,2)) or '', titulo_1)
				else:
