...
class AccountMoveCompensationReceivableLine(models.Model):
	_name = 'account.move.compensation.receivable.line'
	_description = "Documentos por Cobrar a Compensar"

	account_move_compensation_receivable_id = fields.Many2one('account.move.compensation',string="Cuentas por Cobrar a Compensar",
	...
	amount_residual_company_currency= fields.Monetary(string="Saldo en MN", currency_field="company_currency_id",
		compute="compute_campo_amounts",store=True)
	amount_compensation = fields.Monetary(string="Monto a Compensar",currency_field="invoice_currency_id")
	...
	@api.depends('account_move_compensation_receivable_id','account_move_compensation_receivable_id.company_id')
	def compute_campo_company_currency_id(self):
		for rec in self:
			if rec.account_move_compensation_receivable_id and rec.account_move_compensation_receivable_id.company_id:
				rec.company_currency_id = rec.account_move_compensation_receivable_id.company_id.currency_id or False
			else:
	...
	@api.depends('account_move_compensation_receivable_id','account_move_compensation_receivable_id.compensation_date',
	...
	def compute_campo_amount_compensation_company_currency(self):
		for rec in self:
			if rec.invoice_currency_id and (rec.invoice_currency_id != rec.company_currency_id) and rec.account_move_compensation_receivable_id.compensation_date:
				...
				rec.amount_compensation_company_currency = abs(rec.invoice_currency_id._convert(rec.amount_compensation or 0.00,rec.company_currency_id,
					self.env['res.company']._company_default_get('account.invoice'), rec.account_move_compensation_receivable_id.compensation_date))
			else:
