...
class TipoDetraccion(models.Model):
	...
	line_ids = fields.One2many('tipo.detraccion.line', 'detraccion_id',
...
class DetraccionDetalle(models.Model):
	...
	product_id = fields.Many2one('product.product', string="Producto y/o Servicio")
	...
	sql_constraints = [
		('producto','unique(product_id)','Este Bien/Servicio ya se encuentra registrado !'),
	]


	######## hacer restriccion para que un producto no pertenesca a mas de un rubro/concepto !!