...
class WizardPaidRetention(models.TransientModel):
	...
	amount_retention_paid_MN = fields.Monetary(string="Monto Retenido en MN",
		currency_field="retention_company_currency_id",
		compute="compute_retention_amounts",store=True)

	paid_journal_id = fields.Many2one('account.journal',string="Diario de Cobro",required=True,
	...
	nro_receipt_retention=fields.Char(string="Nro Comprobante Retención",required=True)
	...
	retention_percentaje=fields.Float(string="Porcentaje de Retención %")
	...
	@api.depends('retention_company_currency_id','paid_currency_id','amount_paid_currency',
		'retention_date','invoice_id.company_id','invoice_aml_id.company_id','retention_percentaje')
	def compute_retention_amounts(self):
		for rec in self:
			rec.amount_retention_paid_MN = 0.00
			rec.amount_paid_MN = 0.00

			if rec.paid_currency_id and (rec.paid_currency_id != rec.retention_company_currency_id) and rec.retention_date:
				rec.amount_retention_paid_MN = abs(rec.paid_currency_id._convert(rec.amount_paid_currency*rec.retention_percentaje*0.01,
				...
				rec.amount_retention_paid_MN=rec.amount_paid_currency*rec.retention_percentaje*0.01
				rec.amount_paid_MN = rec.amount_paid_currency


	###########################################################
	def add_register_retention_in_account_move(self):
		if self.retention_id and self.invoice_aml_id:
			move_id = self.invoice_aml_id.move_id 
			if move_id:
				move_id.account_invoice_retention_ids += self.retention_id
		...
		if (self.invoice_id and self.invoice_id.invoice_payment_state != 'paid') or \
			...
			if self.retention_company_currency_id and self.paid_currency_id and self.retention_company_currency_id != self.paid_currency_id:
				debit, credit, amount_currency, currency_id = new_account_move_line.with_context(date=self.retention_date).\
					_compute_amount_fields(self.amount_paid_currency*self.retention_percentaje*0.01,
						self.paid_currency_id, self.retention_company_currency_id)
			else:
				debit=self.amount_paid_currency*self.retention_percentaje*0.01
				credit=0.00
				amount_currency=debit-credit
				currency_id=self.retention_company_currency_id
			...
			retention_move_line_id = new_account_move_line.create(self._get_shared_move_line_vals(
				account_id=self.retention_journal_id.default_account_id,
			...
			liquidez_move_line_id = new_account_move_line.create(self._get_shared_move_line_vals(
				account_id=self.paid_journal_id.default_account_id,
				name=self.retention_ref or '',
				debit=self.amount_paid_MN - debit,
				credit=credit,
				currency_id=currency_id,
				amount_currency=self.amount_paid_currency*(1-self.retention_percentaje*0.01) if self.retention_company_currency_id \
					and self.paid_currency_id and self.retention_company_currency_id != self.paid_currency_id else (self.amount_paid_MN - debit-credit)))			
			...
			if not self.paid_currency_id.is_zero(self.amount_paid_MN):
				...
				liquidity_aml_dict = self._get_shared_move_line_vals(
					...
					amount_currency=-self.amount_paid_currency if self.retention_company_currency_id and self.paid_currency_id \
						and self.retention_company_currency_id != self.paid_currency_id else (credit - self.amount_paid_MN)
					)
			...
			"""if self.invoice_id:
				self.invoice_id.account_invoice_retention_line_ids.create({
					...
					'nro_receipt_retention':self.nro_receipt_retention
					})"""
			if self.retention_id:
				self.retention_id.account_retention_line_ids.create({
					...
					'retention_move_id':self.retention_move_id.id,
					'nro_receipt_retention':self.nro_receipt_retention,
					'paid_journal_id':self.paid_journal_id and self.paid_journal_id.id or False,
			...
			######################################
			self.add_register_retention_in_account_move()
	...
	def _get_shared_move_line_vals(self,account_id,name,debit,credit,amount_currency,currency_id):
		line_vals = {
			...
			'name': name or '',
			'currency_id':(currency_id and currency_id.id) or (self.retention_company_currency_id and self.retention_company_currency_id.id),
			'date_maturity':self.retention_date or False,
			'is_retention':True,
			'invoice_retention_id':self.retention_id.id or False}
