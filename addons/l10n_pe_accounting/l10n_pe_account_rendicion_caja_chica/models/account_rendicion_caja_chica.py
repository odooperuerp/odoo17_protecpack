...
class AccountRendicionCajaChica(models.Model):
	_inherit = ['mail.thread', 'mail.activity.mixin']
	_name = 'account.rendicion.caja.chica'
	...
	rendicion_caja_chica_line_ids = fields.One2many('account.rendicion.caja.chica.line','rendicion_caja_chica_id',
		string="Documentos de Gastos a Rendir")
	
	
	rendicion_caja_chica_gastos_sindocumento_line_ids = fields.One2many('account.rendicion.caja.chica.gastos.sindocumento.line',
	...
	provision_payment_id=fields.Many2one('account.payment',string="Provisión de Caja Chica", 
	...
	provision_account_id = fields.Many2one('account.account',string="Cuenta Provisión Caja Chica",
		required=True,domain="[('account_type', 'in',['asset_current'])]")
	...
	###########################################################################################
	amount_rendido_company_currency = fields.Monetary(
		string="Gastos Rendidos con Documentos en MN",
		currency_field="company_currency_id",
		compute="compute_campo_amount_rendido_company_currency",store=True)

	
	amount_rendido_sin_documentos_company_currency = fields.Monetary(
		string="Gastos Rendidos sin Documentos en MN",
		currency_field="company_currency_id",
	...
	@api.constrains('rendicion_caja_chica_line_ids','rendicion_caja_chica_line_ids.invoice_aml_id')
	def _check_campo(self):
		for record in self:
			if record.rendicion_caja_chica_line_ids:
				list_aml_id = [i.invoice_aml_id for i in record.rendicion_caja_chica_line_ids]
				if list_aml_id:
	...
	@api.onchange('rendicion_caja_chica_line_ids','rendicion_caja_chica_line_ids.invoice_aml_id')
	...
	def compute_campo_name(self):
		for rec in self:
			if rec.provision_payment_id:
				rec.name = "Rendición de Caja Chica %s"%(rec.provision_payment_id.name or rec.rendicion_move_id.name or '')
			else:
				rec.name = "Rendición de Caja Chica Nueva"


	def name_get(self):
		result = []
		for rec in self:
			result.append((rec.id,"Rendición de Caja Chica %s"%(rec.provision_payment_id.name or rec.rendicion_move_id.name or '')))
		return result
	...
	def onchange_provision_account_id(self):
		if self.provision_payment_id and self.partner_id:
			...
			move_line_ids = self.provision_payment_id.mapped('move_id.line_ids').\
				filtered(lambda h : h.account_id != self.company_id.transfer_account_id and \
					h.partner_id == self.partner_id and )
	...
	def onchange_provision_account_move_line_id(self):
		if self.provision_payment_id and self.provision_account_id and self.partner_id:
			...
			move_line_ids = self.provision_payment_id.mapped('move_id.line_ids').filtered(
				lambda h : h.account_id ==self.provision_account_id and h.partner_id == self.partner_id)
	...
	@api.depends('rendicion_caja_chica_line_ids',
		'rendicion_caja_chica_line_ids.amount_total_rendir_company_currency')
	def compute_campo_amount_rendido_company_currency(self):
		for rec in self:
			if rec.rendicion_caja_chica_line_ids:
				rec.amount_rendido_company_currency = sum(rec.rendicion_caja_chica_line_ids.mapped('amount_total_rendir_company_currency'))


	
	@api.depends('rendicion_caja_chica_gastos_sindocumento_line_ids',
		'rendicion_caja_chica_gastos_sindocumento_line_ids.amount_total_company_currency')
	def compute_campo_amount_rendido_sin_documentos_company_currency(self):
		for rec in self:
			if rec.rendicion_caja_chica_gastos_sindocumento_line_ids:
				rec.amount_rendido_sin_documentos_company_currency = \
					sum(rec.rendicion_caja_chica_gastos_sindocumento_line_ids.mapped('amount_total_company_currency'))
	...
	def rendir_gastos(self):
		...
		for line in self.rendicion_caja_chica_line_ids:
		...
		for line in self.rendicion_caja_chica_gastos_sindocumento_line_ids:
	...
	def button_view_move_id(self):
		if self.state == 'send':
			return {
				'name': 'Asiento Contable Rendición de Caja Chica',
				'view_type': 'form',
				'view_mode': 'tree,form',
	...
	def button_view_invoice_ids(self):
		if self.state == 'send':
			invoice_ids = self.rendicion_caja_chica_line_ids.mapped('invoice_id.id')
			if invoice_ids:
				return {
					'name': 'Facturas de Gastos Rendidos',
					'view_type': 'form',
					'view_mode': 'tree,form',
					'res_model': 'account.move',
					'view_id': False,
